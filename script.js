...
function handleCardClick(event) {
  ...
  cardClick.style.backgroundColor = cardClick.classList[0];
    if(!cardOne || !cardTwo){
      ...
      cardOne = cardOne || cardClick;
      cardTwo = cardClick === cardOne ? null : cardClick;
  }
    if(cardOne && cardTwo){
      noClicking = true;
    ...
    if (cardsFlipped === COLORS.length) alert("ALL MATCHED!");
}

  
  // function matchCards(cardOne, cardTwo){
  //   console.log(cardOne.class, cardTwo.class);
  // }
//should only be able to click two cards at a time
